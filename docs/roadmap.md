...
| DAN-182 Simplify CLI into four primary flows plus optional UI | In Progress | Current alpha candidate | CLI ergonomics | `scripts/hyperagent.sh`, `docs/quickstart.md`, `evals/cli-help-smoke.sh` | Review grouped `mission` and `review` commands plus compatibility aliases before promoting the simplified CLI surface. |
