...
- Final outcome: Added a documented `scripts/setup-codex.sh` setup path and `bin/hyperagent setup-codex` wrapper route that clone/update HyperAgent, run verification, install/update the Codex skill, and optionally initialize a target project only after confirmation.
...
- Summary of actions taken: Added `scripts/setup-codex.sh`; exposed it through `scripts/hyperagent.sh setup-codex`; documented the one-command path in README and quickstart; expanded clean-install UAT; added `evals/setup-codex-smoke.sh`; updated `scripts/verify-mvp.sh` and eval docs.
...
- Verification performed: syntax checks, setup smoke, MVP verifier, temp skills installer smoke, init smoke, smoke loop.

## Friction

- Failures, retries, and blockers: Initial setup smoke caught an unbound shell variable in `scripts/verify-mvp.sh`; fixed by matching the literal `$install_dir` text with single quotes. No external blocker.
